...
def visualize_attention(model, test_loader, device, model_name: str, output_dir: str):
    ...
    plt.suptitle(f'Attention Maps — {model_name}\n(Top: image | Bottom: attention)',
                 fontsize=12)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'{model_name}_attention.png'),
...
def print_comparison_table(results: list):
    print(f'\n{"="*70}')
    print(f'{"Model":<35} {"Acc(%)":>7} {"Size(MB)":>10} {"Time(min)":>11} {"Params(M)":>11}')
    print(f'{"─"*70}')
    for r in results:
        print(f'{r["model"]:<35} {r["test_acc"]:>7.2f} '
