...
def train_one_epoch(
    model, loader, optimizer, criterion, scaler, device, scheduler=None
):
    model.train()
...
def train(cfg: dict, model_name: str, device: torch.device, train_loader, val_loader):
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        print(
            f"{epoch:>6} {tr_loss:>8.4f} {tr_acc*100:>6.2f}% "
    ...
    with open(os.path.join(cfg["output_dir"], f"{model_name}_log.json"), "w") as f:
